    ...
    for ep in range(1, epoch_num+1):
        ...
        for i, sample in enumerate(train_audio_loader):
            ...
            if global_step < pre_step:
                ...
                fragile_sum_loss = lambda_m * fragile_losses[1]
            else:
                sum_loss = lambda_e*losses[0] + lambda_m*losses[1]
                fragile_sum_loss = lambda_e * fragile_losses[0] + lambda_m * fragile_losses[1]
            ...
            if train_config["adv"]:
                ...
                d_on_fragile_encoded_for_enc = discriminator(fragile_encoded)
                # target label for encoded images should be 'cover', because we want to fool the discriminator

                g_loss_adv = F.binary_cross_entropy_with_logits(d_on_encoded_for_enc, g_target_label_encoded)
                g_fragile_loss = F.binary_cross_entropy_with_logits(d_on_fragile_encoded_for_enc, g_target_label_encoded)

                sum_loss += lambda_a*g_loss_adv
                fragile_sum_loss += lambda_a * g_fragile_loss
            ...
            if step % show_circle == 0:
                ...
                fragile_decoder_acc = [((fragile_decoded[0] >= 0).eq(fragile_msg >= 0).sum().float() / fragile_msg.numel()).item(), ((fragile_decoded[1] >= 0).eq(fragile_msg >= 0).sum().float() / fragile_msg.numel()).item()]
                ...
                fragile_snr = 10 * torch.log10(mse_loss(wav_matrix.detach(), zero_tensor) / mse_loss(wav_matrix.detach(), fragile_encoded.detach()))
                
                norm2=mse_loss(wav_matrix.detach(),zero_tensor)
                print('-' * 100)
                print("step:{} - wav_loss:{:.8f} - msg_loss:{:.8f} - acc:[{:.8f},{:.8f}] - snr:{:.8f} - norm:{:.8f} - patch_num:{} - pad_num:{} - wav_len:{} - d_loss_on_encoded:{} - d_loss_on_cover:{}".format(\
                    step, losses[0], losses[1], fragile_decoder_acc[0], fragile_decoder_acc[1],\
                    fragile_snr, norm2, sample["patch_num"].item(), sample["pad_num"].item(),\
                    wav_matrix.shape[2], d_loss_on_encoded, d_loss_on_cover))
                
                print('-' * 100)
                print("step:{} - fragile_wav_loss:{:.8f} - fragile_msg_loss:{:.8f} - fragile_acc:[{:.8f},{:.8f}] - fragile_snr:{:.8f} - norm:{:.8f} - patch_num:{} - pad_num:{} - wav_len:{} - d_loss_on_encoded:{} - d_loss_on_cover:{}".format(\
                    step, fragile_losses[0], fragile_losses[1], decoder_acc[0], decoder_acc[1],\
                ...
                wandb.log({'fragile_wav_loss': fragile_losses[0]})
                wandb.log({'fragile_msg_loss': fragile_losses[1]})
                wandb.log({'fragile_snr': fragile_snr})"""
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(val_audios_loader):
                ...
                no_attack_msg_loss += wm_losses[2]
            ...
            """wandb.log({'valid_watermark_msg_loss_attack': wm_avg_acc[0]})
            wandb.log({'valid_watermark_msg_loss': wm_avg_acc[1]})
            wandb.log({'valid_unwatermark_msg_loss': unwm_avg_acc[0]})
            wandb.log({'attack_real_msg_loss': wm_avg_msg_loss})
            wandb.log({'no_attack_real_msg_loss': no_attack_msg_loss})"""
            
            
            print('#e' * 60)
            print("epoch:{} - wm_wav_loss:{:.8f} - unwm_wav_loss:{:.8f} - wm_msg_loss:{:.8f} - unwm_msg_loss:{:.8f} - wm_acc:[{:.8f},{:.8f}] - unwm_acc:[{:.8f},{:.8f}] - snr:{:.8f} - d_loss_on_encoded:{} - d_loss_on_cover:{}".format(\
