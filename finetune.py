...
def finetune(args, configs):
    ...
    params_to_optimize_decoder = chain(learnable_parameters_decoder["params"], learnable_parameters_decoder["params_1d"])

    if train_config["finetune_both_encoder_decoder"] == True:
        # Decomposing the weight matrix using SVD
        finetune_optim = Adam(
            params = [{"params": params_to_optimize_encoder}, {"params": params_to_optimize_decoder}],
        ...
        finetune_optim = Adam(
            params = params_to_optimize_decoder,
            betas = train_config["optimize"]["betas"],
            eps = train_config["optimize"]["eps"],
            weight_decay=train_config["optimize"]["weight_decay"],
            lr = train_config["optimize"]["lr"]
