...
class WatermarkExtracter(nn.Module):
    def __init__(self, input_channel=1, hidden_dim=64, block='skip', n_layers=6):
        ...
        post_layers = []
        for i in range(n_layers-2):
            post_layers.append(core(c_in=hidden_dim, c_out=hidden_dim, kernel_size=3, stride=1, padding=1))

        post_layers.append(core(c_in=hidden_dim, c_out=1, kernel_size=3, stride=1, padding=1))
    ...
    def forward(self, x):
        ...
        post_output = self.post_process(pre_output)
        return post_output
