...
class WatermarkExtracter(nn.Module):
    def __init__(self, input_channel=1, hidden_dim=64, lstm_dim= 128, block='skip', n_layers=6):
        ...
        if block == 'skip':
            ...
            raise ValueError(f"Invalid block type: {block}")
        pre_layers = [core(c_in=input_channel, c_out=hidden_dim, kernel_size=3, stride=1, padding=1)]
        LSTM_layers = [lstm_core(input_channel=hidden_dim, output_dim=lstm_dim, num_layers=n_layers - 2)]

        post_layers = []
        for i in range(n_layers-2):
            post_layers.append(core(c_in=lstm_dim, c_out=lstm_dim, kernel_size=3, stride=1, padding=1))

        post_layers.append(core(c_in=lstm_dim, c_out=1, kernel_size=3, stride=1, padding=1))

        self.pre_process = nn.Sequential(*pre_layers)
        self.LSTM_process = nn.Sequential(*LSTM_layers)
        self.post_process = nn.Sequential(*post_layers)

    def forward(self, x):
        pre_output = self.pre_process(x)
        lstm_output = self.LSTM_process(pre_output.squeeze().transpose(0, 2)).transpose(0, 2).unsqueeze(0)
        post_output = self.post_process(lstm_output)
        return post_output
