...
def finetune(args, configs):
    ...
    robust_decoder = Decoder(process_config, model_config, msg_length // 2, win_dim, embedding_dim, nlayers_decoder=nlayers_decoder, attention_heads=attention_heads_decoder).to(device)
    fragile_decoder = Decoder(process_config, model_config, msg_length // 2, win_dim, embedding_dim, nlayers_decoder=nlayers_decoder, attention_heads=attention_heads_decoder).to(device)
    ...
    if train_config["finetune_both_encoder_decoder"] == True:
        ...
        finetune_robust_optim = Adam(
            params = chain(encoder.parameters(), robust_decoder.parameters()),
            betas = train_config["optimize"]["betas"],
            eps = train_config["optimize"]["eps"],
            weight_decay=train_config["optimize"]["weight_decay"],
            lr = train_config["optimize"]["lr"]
        ...
        finetune_fragile_optim = Adam(
            params = chain(encoder.parameters(), fragile_decoder.parameters()),
            betas = train_config["optimize"]["betas"],
            eps = train_config["optimize"]["eps"],
            weight_decay=train_config["optimize"]["weight_decay"],
            lr = train_config["optimize"]["lr"]
        ...
        finetune_robust_optim = Adam(
            params = robust_decoder.parameters(),
            betas = train_config["optimize"]["betas"],
            eps = train_config["optimize"]["eps"],
            weight_decay=train_config["optimize"]["weight_decay"],
            lr = train_config["optimize"]["lr"]
        ...
        finetune_fragile_optim = Adam(
            params = fragile_decoder.parameters(),
            betas = train_config["optimize"]["betas"],
            eps = train_config["optimize"]["eps"],
            weight_decay=train_config["optimize"]["weight_decay"],
            lr = train_config["optimize"]["lr"]
    ...
    lr_robust_sched = StepLR(finetune_robust_optim, step_size=train_config["optimize"]["step_size"], gamma=train_config["optimize"]["gamma"])
    lr_fragile_sched = StepLR(finetune_fragile_optim, step_size=train_config["optimize"]["step_size"], gamma=train_config["optimize"]["gamma"])
    # -------------- Loss 
    ...
    for ep in range(1, epoch_num + 1):
        ...
        for i, sample in enumerate(train_audio_loader):
            ...
            my_step(finetune_robust_optim, lr_robust_sched, global_step, train_len)
        ...
        if ep % save_circle == 0:
            ...
            save_multi_decoder_finetune_op(path, ep, encoder, robust_decoder, fragile_decoder, discriminator, finetune_robust_optim, finetune_fragile_optim, train_config["attack_type"])
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(val_audios_loader):
                ...
                eval_wm_losses = loss.multi_de_loss(wav_matrix, encoded, msg, robust_decoded[0], robust_decoded[1], fragile_decoded[0], fragile_decoded[1])
                eval_unwm_losses = loss.multi_de_loss(wav_matrix, encoded, msg, unwm_robust_decoded[0], unwm_robust_decoded[1], unwm_fragile_decoded[0], unwm_fragile_decoded[1])
                ...
                unwm_rec_msg = torch.cat((unwm_robust_decoded[1], unwm_fragile_decoded[1]), dim = 2)
